...
cd /lucky-blockchain || exit 1

# shellcheck disable=SC1091
. ./activate

lucky init
# lucky init --fix-ssl-permissions

if [[ ${testnet} == 'true' ]]; then
  echo "configure testnet"
  lucky configure --testnet true
fi
...
lucky configure --upnp "${upnp}"

if [[ -n "${log_level}" ]]; then
  lucky configure --log-level "${log_level}"
fi

if [[ -n "${peer_count}" ]]; then
  lucky configure --set-peer-count "${peer_count}"
fi

if [[ -n "${outbound_peer_count}" ]]; then
  lucky configure --set_outbound-peer-count "${outbound_peer_count}"
fi
